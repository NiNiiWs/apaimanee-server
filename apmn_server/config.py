...
class Configuration:
    ...
    def __parse(self):
        ...
        sections = ['apmn', 'DEFAULT']

        boolean_conf    = ['debug']
        integer_conf    = ['mqtt.port']
        ...
        if not 'mongodb.host' in self.settings:
            self.settings['mongodb.host'] = 'localhost'
        if not 'mongodb.db_name' in self.settings:
            self.settings['mongodb.db_name'] = 'apmn'
